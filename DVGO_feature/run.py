...
def config_parser():
    ...
    parser.add_argument("--export_coarse_only", type=bool, default=True)
...
def mse2(raw_feature, target, alpha, ray_id):
    target = target[ray_id]

    loss = (torch.abs(raw_feature - target)).sum(-1)
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    last_ckpt_path = os.path.join(cfg.basedir, cfg.expname, f'{stage}_100000.tar')
    if args.no_reload:
    ...
    def gather_training_rays():
        if data_dict['irregular_shape']:
            ...
            feature_tr_ori = [features[i] for i in i_train]
        else:
            rgb_tr_ori = images[i_train].to('cpu' if cfg.data.load2gpu_on_the_fly else device)
            # feature_tr_ori = feature_tr_ori

        if cfg_train.ray_sampler == 'in_maskcache':
            rgb_tr, feature_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = dvgo.get_training_rays_in_maskcache_sampling(
                    rgb_tr_ori=rgb_tr_ori,
                    feature_tr_ori=features,
            ...
            print(rgb_tr_ori.shape, features.shape)
            rgb_tr, feature_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = dvgo.get_training_rays_flatten(
                rgb_tr_ori=rgb_tr_ori,
                feature_tr=features,
                train_poses=poses[i_train],
            ...
            rgb_tr, feature_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = dvgo.get_training_rays(
                rgb_tr=rgb_tr_ori,
                feature_tr=features,
                train_poses=poses[i_train],
                HW=HW[i_train], Ks=Ks[i_train], ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
        ...
        return rgb_tr, feature_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, batch_index_sampler

    rgb_tr, feature_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, batch_index_sampler = gather_training_rays()
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if cfg_train.ray_sampler in ['flatten', 'in_maskcache']:
            ...
            target = rgb_tr[sel_i]
            target_feature = torch.from_numpy(feature_tr[sel_i])
            ...
            target_feature = torch.from_numpy(feature_tr[sel_b, sel_r, sel_c])
        ...
        loss_feature = cfg_train.weight_main * F.l1_loss(render_result['feature_marched'], target_feature)
        raw_feature = render_result['raw_feature']
        loss_contrastive = cfg_train.weight_main * mse2(render_result['raw_feature'], target_feature, render_result['weights'], render_result['ray_id'])
        print (loss, loss_feature, loss_contrastive)
        ...
        if global_step%args.i_weights==0:
            path = os.path.join(cfg.basedir, cfg.expname, f'{stage}_{global_step:06d}_nps.tar')
            torch.save({
...
if __name__=='__main__':
    ...
    if True:
        ...
        with torch.no_grad():
            ckpt_path = os.path.join(cfg.basedir, cfg.expname, 'fine_120000_nps.tar')
            model = utils.load_model(dcvgo.DirectContractedVoxGO, ckpt_path).to(device)
            model.density.scale_volume_grid(torch.tensor([240, 240, 240]))
            alpha = model.activate_density(model.density.get_dense_grid()).squeeze().cpu().numpy()
            model.k1.scale_volume_grid(torch.tensor([240, 240, 240]))
            rgb2 = model.k1.get_dense_grid().squeeze().permute(1,2,3,0).reshape((240*240*240, 128))
            alpha2 = alpha.reshape(240*240*240)

            rgb2 = rgb2[alpha2>0.5]
            bs = 4096
            top = 0
            features = np.zeros((rgb2.shape[0], feature_dim_global), dtype=np.float16)
            print ("start pridicting features")
            while top + bs < alpha2.shape[0] + bs:
                print (top / (alpha2.shape[0]) * 100)
                features[top:top+bs] = model.featurenet2_new(rgb2[top:top+bs].cuda()).cpu().numpy().astype(np.float16)
                top += bs

        testsavedir = os.path.join(cfg.basedir, cfg.expname, "%s_nps.npz"%cfg.expname)
        np.savez_compressed(testsavedir, alpha=alpha, features=features)
        print('done')
        sys.exit()
