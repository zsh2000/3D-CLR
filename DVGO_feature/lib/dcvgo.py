...
class DirectContractedVoxGO(nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        if rgbnet_dim <= 0:
            ...
            self.k1 = grid.create_grid(
                k0_type, channels=128, world_size=torch.tensor([120,120,120]),
                xyz_min=self.xyz_min, xyz_max=self.xyz_max,
                config=self.k0_config)

            self.featurenet2_new = nn.Sequential(
                nn.Linear(128, feature_dim_global), nn.ReLU(inplace=True),
                *[
                    nn.Sequential(nn.Linear(feature_dim_global, feature_dim_global), nn.ReLU(inplace=True))
                    for _ in range(1)
                ],
                nn.Linear(feature_dim_global, feature_dim_global),
            )

            nn.init.constant_(self.rgbnet[-1].bias, 0)
            nn.init.constant_(self.featurenet2_new[-1].bias, 0)
            print('dcvgo: feature voxel grid', self.k0)
            print('dcvgo: mlp', self.rgbnet)
            print('dcvgo: featuremlp', self.rgbnet)
    ...
    def scale_volume_grid(self, num_voxels):
        ...
        self.k1.scale_volume_grid(self.world_size)
    ...
    def k0_total_variation_add_grad(self, weight, dense_mode):
        ...
        self.k1.total_variation_add_grad(w, w, w, dense_mode)
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, is_train=False, **render_kwargs):
        ...
        if self.rgbnet is None:
            ...
            feature = self.featurenet2_new(k1)
            # feature = torch.sigmoid(feature_logit)
        ...
        feature_marched = segment_coo(
                src=(weights.unsqueeze(-1) * feature),
                index=ray_id,
                out=torch.zeros([N, feature_dim_global]),
                reduce='sum')
        ...
        ret_dict.update({
            ...
            'feature_marched': feature_marched,
            'raw_density': density,
            'raw_alpha': alpha,
            'raw_rgb': rgb,
            'raw_feature': feature,
