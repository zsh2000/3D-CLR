...
@torch.no_grad()
def get_training_rays(rgb_tr, feature_tr, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y):
    ...
    return rgb_tr, feature_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz


@torch.no_grad()
def get_training_rays_flatten(rgb_tr_ori, feature_tr, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y):
    ...
    feature_tr = feature_tr.reshape((N, feature_dim_global))
    # feature_tr = np.zeros((N,512), dtype=np.float16)
    # feature_tr = torch.zeros([N,512], device=DEVICE)
    ...
    for c2w, img, (H, W), K in zip(train_poses, rgb_tr_ori,  HW, Ks):
        ...
        # feature_tr[top:top+n] = feature.flatten(0,1)
        # feature_tr[top:top+n].copy_(feature.flatten(0,1))
...
@torch.no_grad()
def get_training_rays_in_maskcache_sampling(rgb_tr_ori, feature_tr_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y, model, render_kwargs):
    ...
    N = sum(im.shape[0] * im.shape[1] for im in rgb_tr_ori)
    rgb_tr = torch.zeros([N,3], device=DEVICE)
    # feature_tr = torch.zeros([N,512], device=DEVICE, dtype=torch.float16)
    feature_tr = np.zeros((N,feature_dim_global), dtype=np.float16)
    ...
    imsz = []
    top = 0
    for c2w, img, feature, (H, W), K in zip(train_poses, rgb_tr_ori, feature_tr_ori, HW, Ks):
        assert img.shape[:2] == (H, W)
        rays_o, rays_d, viewdirs = get_rays_of_a_view(
                H=H, W=W, K=K, c2w=c2w, ndc=ndc,
                inverse_y=inverse_y, flip_x=flip_x, flip_y=flip_y)
        ...
        # feature_tr[top:top+n].copy_(feature[mask])
        feature_tr[top:top+n] = feature[mask.numpy()]
