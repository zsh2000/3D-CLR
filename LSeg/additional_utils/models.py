...
class LSeg_MultiEvalModule(DataParallel):
    ...
    def parallel_forward(self, inputs, label_set='', **kwargs):
        ...
        features, text_features, outputs = parallel_apply(replicas, inputs, label_set, kwargs)
        return features, text_features, outputs
    ...
    def forward(self, image, label_set=''):
        ...
        with torch.cuda.device_of(image):
            ...
            feature_totals = image.new().resize_(batch,512,h,w).zero_().cuda()
        ...
        for scale in self.scales:
            ...
            if long_size <= crop_size:
                ...
                features, text_features, outputs = module_inference(self.module, pad_img, label_set, self.flip)
                
                outputs = crop_image(outputs, 0, height, 0, width)
                features = crop_image(features, 0, height, 0, width)
                ...
                with torch.cuda.device_of(image):
                    ...
                    features = image.new().resize_(batch,512,ph,pw).zero_().cuda()
                ...
                for idh in range(h_grids):
                    for idw in range(w_grids):
                        ...
                        pad_crop_img = pad_image(crop_img, self.module.mean,
                                                 self.module.std, crop_size)
                        feature, text_feature, output = module_inference(self.module, pad_crop_img, label_set, self.flip)
                        
                        outputs[:,:,h0:h1,w0:w1] += crop_image(output,
                            0, h1-h0, 0, w1-w0)

                        features[:,:,h0:h1,w0:w1] += crop_image(feature,
                            0, h1-h0, 0, w1-w0)
                ...
                features = features / count_norm
                outputs = outputs[:,:,:height,:width]
                features = features[:,:,:height,:width]

            score = resize_image(outputs, h, w, **self.module._up_kwargs)

            feature_total = resize_image(features, h, w, **self.module._up_kwargs)

            scores += score

            # if scale == 1.0:
            #     scores = score
            #     feature_totals = feature_total
            feature_totals += feature_total

        feature_totals /= len(self.scales)
        
        return feature_totals, text_features, scores

def module_inference(module, image, label_set, flip=True):
    ffeature, ftext_feature, output = module.evaluate_random(image, label_set)
    if flip:
        fimg = flip_image(image)
        ffeature, ftext_feature, foutput = module.evaluate_random(fimg, label_set)
        output += flip_image(foutput)
        ffeature += flip_image(ffeature)

    return ffeature, ftext_feature, output
...
def parallel_apply(modules, inputs, label_set, kwargs_tup=None, devices=None):
        ...
        try:
            with torch.cuda.device(device), autocast(enabled=autocast_enabled):
                ...
                if not isinstance(input, (list, tuple)):
                    input = (input,)
                feature, text_feature, output = module(*input, label_set, **kwargs)
            with lock:
                results[i] = (feature, text_feature, output)
        except Exception:
            with lock:
    ...
    for i in range(len(inputs)):
        if isinstance (results[i], ExceptionWrapper):
            results[i].reraise()
        feature, text_feature, output = results[i]
        if isinstance(output, ExceptionWrapper):
            output.reraise()
        if isinstance(feature, ExceptionWrapper):
            output.reraise()
